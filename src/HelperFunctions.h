...
{
	template<typename T>
	inline std::vector<T> sub_vector(std::vector<T> const& v, int m, int n) {
	...
	inline vec3 points_to_vector(vec3 v1, vec3 v2) {
		return vec3(v2.x - v1.x, v2.y - v1.y, v2.z - v1.z);
	}

	inline vec2 coord_after_rotation(vec2 old_position, vec2 rotating_point, float rotation_angle, float new_angle) {
		float x_prime = 0;
		float y_prime = 0;
	...
	inline long double calc_angle(vec3 v1, vec3 v2)
	{
		v1 = normalize(v1);
		v2 = normalize(v2);
